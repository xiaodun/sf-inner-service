      ...
      try {
        ...
        wss.on('connection', function (ws) {
          ws.on('message', function (message) {
            let websocketjs = file_os.readFileSync(
              websocketPath,
              'utf-8',
            );
            // 广播消息
  ...
  var server = http_os.createServer(function (request, response) {
    try {
      ...
      let prefix = urlElementsArr[0],
        ...
        moduleNames = urlElementsArr.slice(
          2,
          urlElementsArr.length - 2,
        ),
        dataName = urlElementsArr[urlElementsArr.length - 2],
        command;
      let paramsPos = urlElementsArr[
        urlElementsArr.length - 1
      ].indexOf('?');
      if (paramsPos == -1) {
        command = urlElementsArr[urlElementsArr.length - 1];
      } else {
        command = urlElementsArr[urlElementsArr.length - 1].slice(
          0,
          paramsPos,
        );
      }
      ...
      rootFloder.commandPath =
        rootFloder.path + '/' + command + '.js';
      ...
      function executeCommand(params) {
        try {
          ...
          //获取json数据
          var data = JSON.parse(
            file_os.readFileSync(rootFloder.dataPath, 'utf-8') ||
              null,
          );
          ...
          if (result.isDelete) {
            ...
            if (lifeCycleModule.getDeleteFilePath) {
              path = lifeCycleModule.getDeleteFilePath(
                external,
                result,
              );
            } else {
          ...
          if (result.isDownload) {
            ...
            if (lifeCycleModule.getDownloadFilePath) {
              path = lifeCycleModule.getDownloadFilePath(
                external,
                result,
              );
            } else {
